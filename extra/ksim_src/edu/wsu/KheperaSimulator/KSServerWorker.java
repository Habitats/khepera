...
/**
 * A <code>KSServerWorker</code> provides the functionality to service a client. This worker conforms to the protocol agreed to by this worker and the client.
 * This worker handles verification of the connection as well as streaming the server data needed by the client for playback.
 * 
...
public class KSServerWorker extends Thread {
	...
	/**
	 * The socket this <code>KSServerThread</code> will use for communication with the client.
	 */
	...
	/**
	 * <code>String</code> objects that contain a status or protocol code which will be sent or received.
	 */
	...
	/**
	 * Indicates if this worker should terminate the connection and quit, releasing all resources.
	 */
	...
	/**
	 * Indicates to this worker to transmit the world map. For now this includes static and moveable objects due to the current design of the simulator.
	 */
	private boolean sendMap = false;

	/**
	 * Amount of time in milliseconds to wait before transmitting the next set of simulator data.
	 */
	private long TX_INTERVAL = 100; // default is 100 ms

	/**
	 * A reference to the local simulator. This allows this worker access to the simulator internals in order to transmit the required data.
	 */
	...
	/**
	 * A reference to the local server in case this worker dies prematurely. This allows the server to keep an updated pool of connected clients.
	 */
	 ...
	 * 
	 * @param frame
	 *            a reference to the local simulator
	 * @param socket
	 *            a <code>Socket</code> object that is connected to a client.
	 * @param workerID
	 * @param TXStatus
	 *            the initial transmit status of this worker
	 * @param server
	 *            a reference to the local server
	 */
	public KSServerWorker(KSFrame frame, Socket socket, String workerID, boolean TXStatus, KSServer server) {
	 ...
	 *            the name of the currently running controller
	 * @param sleepTime
	 *            the time in milliseconds to wait before sending another set of data
	 */
	...
	/**
	 * Tell this <code>KSServerWorker</code> to stop transmitting. The connection will remain open and a call to startTransmission will begin transmission to
	...
	/**
	 * Tell this <code>KSServerWorker</code> to terminate the connection with the client.
	 */
	...
	public void run() {
		try {
			...
			server.removeResource(workerID);
		}
	} // run
	...
	/**
	 * Transmit one set of simulator data to the client, then wait <tt>sleepTime</tt>.
	 */
	...
	/**
	 * Verify the connection with the client by conforming to the request/response protocol.
	 */
	...
	/**
	 * Initializes the streams that this <code>KSServerWorker</code> will use to communicate with the client.
	 */
	...
	/**
	 * Close the socket and the streams associated with this <code>KSServerWorker</code> objects socket.
	 */
