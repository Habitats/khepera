...
/**
 * A <code>KSServer</code> provides the functionality to listen for and service multiple clients. This server object also provides methods to manage the clients
 * once they are being serviced, as well as controlling the communication with the client.
 * 
...
public class KSServer implements Runnable {
	...
	/**
	 * Initialize this server and start listening for clients on the DEFAULT_PORT. A call to this method will spawn the server on a new <code>Thread</code>.
	 */
	...
	/**
	 * Initialize this server and start listening for clients on the specified port. A call to this method will spawn the server on a new <code>Thread</code>.
	 * 
	 * @param frame
	 *            reference to the parent frame
	 * @param port
	 *            desired port for which this server will listen for clients
	 */
	 ...
	 * 
	 * @return return <tt>true</tt> if this server is currently listening for for clients; <tt>false</tt> otherwise.
	 */
	 ...
	 * 
	 * @param workerID
	 *            the worker ID of the worker to remove
	 */
	 ...
	 *            the client name to start transmission
	 * @param interval
	 *            the time in milliseconds at which the service thread will wait until transmission of the next batch of simulator data.
	 */
	...
	/**
	 * Stop transmission of simulator data to all clients. The connections to the clients will remain alive. A subsequent call to <code>startTransmission</code>
	 * will begin transmission of simulator data to the clients once again.
	 */
	...
	/**
	 * Stop this server from listening for clients. Any active client connections will remain active.
	 */
	protected void kill() {
		try {
			setListening(false);
	...
	/**
	 * Set the listening status of this server and display the status in the status bar of the application.
	 * 
	 * @param listening
	 *            <tt>true</tt> if this server is listening
	 */
	...
	/**
	 * Spawn a new <code>KSServerWorker</code> to handle the communication with a client.
	 * 
	 * @param socket
	 *            the <code>Socket</code> to which the client is connected.
	 */
