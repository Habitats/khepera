...
/**
 * The <code>WorldPanel</code> class implements a custom JPanel for drawing world objects and robot animations. This panel also handles mouse events when
 * placing or removing objects for custom world configurations.
 * 
...
public class WorldPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	public void paint(Graphics g) {
		...
		if (!running) {
			...
			if ((dragObject != null) && (buttonClicked == WorldPanel.ADD || buttonClicked == WorldPanel.ROBOT))
	 ...
	 * 
	 * @param state
	 *            true if robot controller is running, else false
	 */
	...
	/**
	 * Sets the <code>heldObjectID</code> field to the id number of any object held in the gripper.
	 * 
	 * @param heldID
	 *            id number of a gripped object, or 0 (no object held)
	 */
	...
	/**
	 * Set what should happen when the mouse is clicked in this panel. Types of "events" are add, remove and move objects. None indicates that no world editing
	 * buttons are selected.
	 * 
	 * @param button
	 *            id number of recently clicked button (id numbers correspond to the named constants ROBOT, ADD, NONE)
	 */
	protected void setEventType(int button) {
		if ((buttonClicked == WorldPanel.ROBOT && button == WorldPanel.ADD) || (buttonClicked == WorldPanel.NONE && button == WorldPanel.ADD)) {
	...
	/**
	 * Set the type of object currently selected in the objectPanel. The default object is a BRICK.
	 * 
	 * @param object
	 *            corresonds to constants WALL, LIGHT, BALL, CAP, ROBOT
	 */
			...
			if (drawObj == v.objType) {
				...
				if (drawObj == WorldPanel.WALL) {
					if (v.theta == 0.0f) { // note: change rotation to 90 or 0
						if (mouseX >= objX && mouseX <= (objX + 7) && mouseY >= objY && mouseY <= (objY + 49)) {
							worldObjects.removeElement(v); // should test ret. val.
							return;
						}
					} else { // v.theta == 270.0
						if (mouseX >= objX && mouseX <= (objX + 49) && mouseY <= objY && mouseY >= (objY - 7)) {
				...
				if (drawObj == WorldPanel.CAP || drawObj == WorldPanel.BALL) {
					if (mouseX >= objX && mouseX <= (objX + 10) && mouseY >= objY && mouseY <= (objY + 10)) {
						worldObjects.removeElement(v);
						return;
					}
				}
				if (drawObj == WorldPanel.LIGHT) {
					if (mouseX >= objX && mouseX <= (objX + 15) && mouseY >= objY && mouseY <= (objY + 15)) {
						worldObjects.removeElement(v);
	...
	public void mouseClicked(MouseEvent e) {
		...
		case ADD:
			Vertex objectNode = new Vertex(drawObj, x, y, theta, idNum);
		...
		case REMOVE:
			...
			break;
		case ROBOT:
			setRobot = new Vertex(WorldPanel.ROBOT, x, y, 0.0f, -1);
			repaint();
			// writeToBuffer();
			break;
		}
	...
	// NEW 7/29/2003 - sp
	/**
	 * Notifies this class that a coordinate change in one or more objects has occured. This method should be replaced by an event generating/listening scheme.
	 * 
	 * @param change
