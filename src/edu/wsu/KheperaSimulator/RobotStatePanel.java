...
/**
 * The <code>RobotStatePanel</code> is a GUI component that shows the current status of the robot gripper, arm, and the object presence sensor. It might be
 * easier to change the implementation of this class so that the images are loaded into JLabels instead of using the graphics API.
 * 
...
public class RobotStatePanel extends JPanel {
	...
	/**
	 * Allocate a new <code>RobotStatePanel</code> that shows the status of the specified type. The possible values for type can be: T_ARM, T_GRIP or T_OBJ
	 * 
	 * @param type
	 *            the type of status
	 */
