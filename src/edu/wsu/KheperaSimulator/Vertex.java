...
/**
 * This class defines a drawable object. The basic members and operations could be used to define class(es) for world objects. A more extensive definition could
 * be used for a robot.
 * 
...
public class Vertex implements Serializable {
	 ...
	 *            world object type
	 * @param _id
	 *            world object id number
	 */
	public Vertex(int type, int _id) {
		this(type, 250, 250, 0.0f, _id);
	}
	 ...
	 *            world object id number
	 */
	public Vertex(int type, int x, int y, int _id) {
		this(type, x, y, 0.0f, _id);
	}
	...
	public Vertex(int type, int x, int y, float a, int _id) {
		...
		objType = type;
		id = _id;
		if (objType == WorldPanel.WALL)
			vertexColor = Color.red;
		else if (objType == WorldPanel.BALL)
			vertexColor = Color.green;
		else if (objType == WorldPanel.CAP)
			vertexColor = Color.blue;
		else if (objType == WorldPanel.LIGHT)
	...
	public void drawVertex(Graphics g) {
		...
		switch (objType) {
		case WorldPanel.WALL:
			g2.rotate(theta, xPos, yPos);
			g2.fillRect(xPos, yPos, 8, 50);
			g2.setTransform(new AffineTransform());
			break;

		case WorldPanel.BALL:
			g2.fillArc(xPos, yPos, 8, 8, 0, 360);
			break;

		case WorldPanel.LIGHT:
			g2.fillArc(xPos, yPos, 15, 15, 0, 360);
			break;

		case WorldPanel.CAP:
			g2.fillArc(xPos, yPos, 8, 8, 0, 360);
			break;
		case WorldPanel.ROBOT:
			rcX = xPos + 10;
			rcY = yPos + 10;
			...
			g2.setTransform(new AffineTransform());
			/*
			 * case WorldPanel.ROBOT:rcX = xPos+10; rcY = yPos+10; g2.rotate(theta, (double)rcX, (double)rcY); g2.setColor(Color.green);
			 * g2.fillArc(xPos,yPos,20,20,0,360); // body g2.setColor(Color.black); g2.drawArc(xPos,yPos,20,20,0,360);
			 * g2.drawLine(xPos,yPos+10,xPos+20,yPos+10);//center g2.fillArc(xPos+15,yPos+8,5,5,0,360); // nose g2.setTransform(new AffineTransform()); //case
			 * WorldPanel.ROBOT: robot = new javax.swi //ng.ImageIcon("roboIcon.gif").getImage();
			 */
		}
	}
