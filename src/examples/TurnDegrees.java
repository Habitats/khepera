 ...
 * 
 * Unlike the other controllers, this version makes sure it has turned the exact number of degrees asked for.
 * 
	...
	public void doWork() throws Exception {
		...
		if (state == 1) {
			...
			if (getLeftWheelPosition() == degreeTarget) {
				...
				if (((getLeftWheelPosition() < degreeTarget) && (theTurnDegree < 0)) || (getLeftWheelPosition() > degreeTarget) && (theTurnDegree > 0)) {
					...
					// System.out.println("TurnDegrees: Wanted to turn to " + degreeTarget + " but turned to" + _ControllerRef.getLeftWheelPosition() +
					// "("+offset+") COMPENSATING!");
