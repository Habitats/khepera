...
class SensorManager extends Thread {
		...
		private ArrayList<LinkedList<Integer>> distanceSensorReadings = new ArrayList<LinkedList<Integer>>();
        ...
        public Point getClosestWall() throws InterruptedException{
    		...
    		int closest = -1;
    		int distance = -1;
    		Sensor[] sensors = {Sensor.LEFT, Sensor.FRONTLEFT, Sensor.FRONTRIGHT, Sensor.RIGHT}; // left, front, front, right
    		...
    		if( distance<14 ){
//    			Sensor sensor = new Sensor[]{Sensor.LEFT, Sensor.FRONTLEFT, Sensor.FRONTRIGHT, Sensor.RIGHT}[new Random().nextInt(4)];
		...
		public int checkBall(int followSensor){
			/*
			 * check for consistent distances, else ball?
			 * - use some kind of measurement history
			 */
			if( followSensor==Sensor.RIGHT.getIndex() && Sensor.LEFTDIAGONAL.distance(distanceSensorData) < ballDistance ){
			...
			else if(followSensor==Sensor.LEFT.getIndex() && Sensor.RIGHTDIAGONAL.distance(distanceSensorData) < ballDistance) {
