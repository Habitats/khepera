...
import old.pat.etc.Direction;
import old.pat.etc.RobotEvent.RobotAction;
...
public abstract class AbstractController extends RobotController {
  ...
  public void doWork() throws Exception {
    ...
    setStatus("Time passed: " + Double.toString((System.currentTimeMillis() - startTime) / 1000.),
        0);
    ...
    setStatus(
        "Direction h or v: "
            + Boolean.toString(approximately(getDirectionInRadians() % (Math.PI / 2), 0., 0.1)), 8);
  ...
  // children need to implement their own way of finding the actual ball, as wall as getting back
  // home
  ...
  private void updateRobotTail() {
    ...
    Singleton.getInstance().getController()
        .drawRobotTail(getCurrentLocation().getNormalized(), state);
  ...
  private void updateMap() {
    if (detectLeft()) {
      ...
      Singleton.getInstance().getController()
          .drawSomething(getOffsetLocation(250, Direction.LEFT).getNormalized());
    }
    if (detectRight()) {
      // map.add(getCurrentLocation());
      Singleton.getInstance().getController()
          .drawSomething(getOffsetLocation(250, Direction.RIGHT).getNormalized());
    }
    if (detectWall()) {
      // map.add(getCurrentLocation());
      Singleton.getInstance().getController()
          .drawSomething(getOffsetLocation(250, Direction.FRONT).getNormalized());
    }
  }
  ...
  private boolean crashing() {
    boolean crash =
        getAverageDistance(SENSOR_FRONTL) > 1000 || getAverageDistance(SENSOR_FRONTR) > 1000;
    if (crash)
