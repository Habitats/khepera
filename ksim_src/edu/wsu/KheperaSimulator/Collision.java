...
/**
 * The <code>Collision</code> class provides methods to check for a collision between the robot and an object in the simulated environment. A collision can
 * occur on the front of the robot or on the rear. This class provides a way to determine which type of collision occurred.
 * 
...
public class Collision {
	...
	/**
	 * Allocates a new <code>Collision</code> object that will detect collisions in the current simulator environment.
	 * 
	 * @param rs
	 *            a reference to the current state of the robot
	 * @param wi
	 *            the simulator environment matrix
	 * @param stuck
	 *            objects that exist in the simulator environment
	 */
	...
	/**
	 * Test for a collision between the current state of the robot and the simulator environment.
	 * 
	...
	protected int testCollision() {
		...
		if (arm == KSGripperStates.ARM_DOWN) {
			if (grip == KSGripperStates.GRIP_OPEN) {
				...
				if (wImage[armTip1Y][armTip1X] == 1 || wImage[armTip1Y][armTip1X] == 2)
					return FRONT_HIT;
				if (testObjects) {
				...
				if (wImage[armTip2Y][armTip2X] == 1 || wImage[armTip2Y][armTip2X] == 2)
					return FRONT_HIT;
				if (testObjects) {
