...
/**
 * A <code>KSWriter</code> class provides the functionality to plug into the local simulator and record the current running controller to a playable file.
 */
public class KSWriter implements Runnable {
	...
	/*
	 * New 7/29/2003 - sp -- these variables have been introduced to allow the recording of dynamic changes in the environment.
	 */
	...
	/**
	 * Initialize this <code>KSWriter</code> object to write at the specified speed.
	 * 
	 * @param frame
	 *            the local simulator
	 * @param the
	 *            delay between frames of data written to file
	 */
		...
		while (record) {
			if (notWritten) {
				try {
					...
					/*
					 * The following is the original header writing routine ** String date = "## " + month + "-" + day + "-" + year; String controller =
					 * "## The current Controller is :" + current; String writeDelay = "## Current Delay is :" + delayTime; out.writeObject(date);
					 * out.writeObject(controller); out.writeObject(writeDelay); out.writeObject(nFrame.worldPanel.worldObjects);
					 */
