...
/**
 * A <code>DrawableRobot</code> class provides methods to calculate a new position of the robot based on the current state of the robot
 */
public class DrawableRobot {
	...
	/**
	 * This value sets maximum speed that the robot can travel in the map. This value must be >= 4.8. The greater this value is the slower the maximum speed of
	 * the robot. For some odd reason this value cannot be any lower than 4.8. Go figure.
	 */
	private double speedMax = 4.8; // found by trial/error

	/**
	 * Allocate a new <code>DrawableRobot</code> object with the current robot state.
	 * 
	 * @param rState
	...
	/**
	 * Reinitialize this <code>DrawableRobot</code> object. This will reset the current state of the robot back to the default state.
	 */
	...
	/**
	 * Update the wheel positions of the robot. Should not update if there is a collision with immobile object.
	 * 
	 * @param lSpeed
	 *            the new speed of the left wheel
	 * @param rSpeed
	 *            the new speed of the right wheel
	 */
	private void updateWheelPositions(int lSpeed, int rSpeed) {
		...
		if (((lp + lSpeed) > Long.MAX_VALUE) || ((lp + lSpeed) < Long.MIN_VALUE))
			lp = 0L;
		else
			lp += lSpeed;
		if (((rp + rSpeed) > Long.MAX_VALUE) || ((rp + rSpeed) < Long.MIN_VALUE))
			rp = 0L;
		else
			rp += rSpeed;
	...
	/**
	 * Initialize the coordinates of this robot to the current state of the <code>RobotCoordinates</code> object.
	 */
	...
	/**
	 * Update the coordinates of the robot based on the current collision status of the robot.
	 * 
	 * @param hit
	 *            collision status of the robot
	 */
