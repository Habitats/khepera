...
/**
 * A <code>KSClient</code> provides the functionality to connect to a server simulator and playback the currently running controller on that server. It is
 * important to note that the client simulator is not running a controller of its own, rather displaying what the server controller is doing.
 * 
...
public class KSClient implements Runnable {
	...
	/**
	 * The socket this <code>KSClient</code> will use for communication with the server.
	 */
	...
	/**
	 * Indicates that this client is connected and currently may or may not be sending or receiving data.
	 */
	private boolean isStarted = false;

	/**
	 * A reference to the local simulator. This allows this client access to the simulator internals in order to set and playback a simulation.
	 */
	...
	/**
	 * Initializes, connects this <code>KSClient</code>, and starts it on a separate thread.
	 * 
	 * @param frame
	 ...
	 * @param port
	 *            the port number that the server is listening
	 */
	...
	/**
	 * Terminate communication with the server. There is no graceful way to tell the server this client is done. However the server is robust enough to recover
	...
	public void run() {
		try {
			...
			JOptionPane.showMessageDialog(null, "Unknown Host: " + host, "Unknown Host", JOptionPane.ERROR_MESSAGE);
			close();
			return;
		} catch (IOException e) {
			JOptionPane.showMessageDialog(null, "IOException: " + host, "IOException", JOptionPane.ERROR_MESSAGE);
			close();
			return;
		}
	...
	/**
	 * Set the status of this client and update the visual status on the status bar.
	 * 
	 * @param isStarted
	 *            indicate if this client is started or not.
	 */
	private void setStarted(boolean isStarted) {
		this.isStarted = isStarted;
		if (isStarted) {
			StatusBar.setLeftStatus("Client connected to: " + clientSocket.getInetAddress().getHostName());
		} else {
	...
	/**
	 * Close the socket and the streams associated with this <code>KSClient</code> objects socket.
	 */
	private void close() {
		try {
			setStarted(false);
